...
const VerticalScroll: React.FC<VerticalScrollProps> = ({
  ...
  // Scroll to max position when content/visible height changes
  useEffect(() => {
    const newMaxScroll = Math.max(0, contentHeight - visibleHeight);
    const clampedPosition = Math.max(0, Math.min(newMaxScroll, newMaxScroll));
    setScrollPosition(clampedPosition);
    onScroll(clampedPosition);
  }, [contentHeight, visibleHeight, onScroll]);
